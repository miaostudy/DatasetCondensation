...
def main():
    ...
    args.mode = 'DSA'
    # setup logger
    log_name = args.mode + '_IPC_{}_limg_{}_Data_{}'.format(args.ipc, args.lr_img, args.dataset)
    # log_path = os.path.join(args.log_dir, log_name)

    logger = build_logger('./log_DSA_base', cfgname=log_name)
    args.logger = logger
        ...
        for it in range(args.Iteration+1):
            ...
            if it in eval_it_pool:
                for model_eval in model_eval_pool:
                    ...
                    logger.info('Evaluate %d random %s, F1mean = %.4f F!std = %.4f\n-------------------------' % (
