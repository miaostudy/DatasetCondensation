...
# import torchvision.models.vision_transformer as VisionTransformer
...
class ConvNet(nn.Module):
    ...
    def _make_layers(self, channel, net_width, net_depth, net_norm, net_act, net_pooling, im_size):
        ...
        for d in range(net_depth):
            ...
            # PathMNIST
            layers += [nn.Conv2d(in_channels, net_width, kernel_size=3, padding=3 if d == 0 else 1)]
...
# class ViTTinyWithEmbedding(models.vit_b_16.VisionTransformer):
# class ViTTinyWithEmbedding(VisionTransformer):
#     def __init__(self, num_classes=1000,image_size = 32, patch_size = 8):
#         super(ViTTinyWithEmbedding, self).__init__(
...
#
#     def embed(self, x):
#         # Vision Transformer 的中间特征通常来自 Transformer 的输出
#         # 可以获取 patch_embed 层的输入或 Transformer 的输出
#         # 以下代码获取 Transformer 的最后一个层的输出
#         B = x.shape[0]
...
#         return x
class ViTTinyWithEmbedding(VisionTransformer):
    def __init__(self, num_classes=1000, image_size=64, patch_size=8):
        super(ViTTinyWithEmbedding, self).__init__(
            img_size=image_size,  # 输入图像大小
    ...
    def embed(self, x):
        ...
        x = x[:, 0]  # 取分类 token 的特征
        return x
# class ConvNextTinyWithEmbedding(convnext):
#     def __init__(self, num_classes=1000):
#         super(ConvNextTinyWithEmbedding, self).__init__(
#             depths=[3, 3, 9, 3], dims=[96, 192, 384, 768], num_classes=num_classes
#         )
#
#     def forward(self, x, return_tuple=False):
#         # 前向传播获得分类结果
...
#     def embed(self, x):
#         # 获取中间特征，例如最后一层卷积层的输出
#         # 可以通过修改 forward_features 函数来获取中间特征
#         x = self.features(x)
#         return x.mean([-2, -1])  # 全局平均池化后的特征