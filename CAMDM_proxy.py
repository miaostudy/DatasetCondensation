...
from utils import train_CAM, get_loops, get_dataset, get_dataset_med, get_network, get_eval_pool, evaluate_synset, get_daparam, match_loss, get_time, TensorDataset, epoch, DiffAugment, ParamDiffAug, load_or_train_model
...
def main():
    ...
    parser.add_argument('--log_dir',type=str,default='./log_0306' )
    ...
    parser.add_argument('--checkpoint_path', type=str, default='/data/users/xiongyuxuan/DD/CAMDM/DatasetCondensation-master/CAM_CKPT', help='dataset path')
    ...
    root = os.getcwd()
    # setup logger
    log_name = args.mode+'_IPC={}_Model_{}_Proxy_{}_Epoch_{}_Eval_{}_CAM_{}'.format(args.ipc,args.model,args.proxy_model,args.epoch_proxy,args.eval_mode, args.cam_type)
    log_path = os.path.join(args.log_dir,log_name)

    logger = build_logger('./log_CAMDM_0908', cfgname=log_name)
    args.logger = logger
    ...
    for exp in range(args.num_exp):
        ...
        elif args.proxy_model == 'ViT-Tiny':
            # target_layer = net_cam.blocks[-1]
            target_layer = net_cam.blocks[-1].norm1


        # 初始化 GradCAM，移除 use_cuda 参数
        if args.cam_type == 'GradCAM':
            if args.proxy_model == 'ViT-Tiny':
                cam = GradCAM(model=net_cam, target_layers=[target_layer],reshape_transform=reshape_transform)
            else:
