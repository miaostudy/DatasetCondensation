        ...
        for it in range(args.Iteration+1):
            ...
            if it in eval_it_pool:
                for model_eval in model_eval_pool:
                    ...
                    print('Evaluate %d random %s, ACCmean = %.4f ACCstd = %.4f\n-------------------------' % (
                    len(accs), model_eval, np.mean(accs), np.std(accs)))
                    print('Evaluate %d random %s, SENmean = %.4f SENstd = %.4f\n-------------------------' % (
                    len(accs), model_eval, np.mean(sens), np.std(sens)))
                    print('Evaluate %d random %s, SPEmean = %.4f SPEstd = %.4f\n-------------------------' % (
                    len(accs), model_eval, np.mean(spes), np.std(spes)))
                    print('Evaluate %d random %s, F1mean = %.4f F1std = %.4f\n-------------------------' % (
